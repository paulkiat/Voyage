/**
 * Listen for app registrations and set up dynamic web proxy
 * from the org web front end server to the app back end web server.
 * This protects the app from direct traffic and allows for a single
...
// allowing shared app services to have app-id context
...
exports.init = function(state) {
    ...
    // For now it's assumed this code runs in the same process space
    ...
    node.subscribe('app-url', (msg, cid) => {
        ...
        log({ app_url_escape: app_id, path });
    ...
    node.subscribe('service-up', async (msg, cid) => {
        ...
        // Handle app web services announcements and add proxy
        ...
        if (handler) {
            ...
            const endpoint = async (req, res, next) => {
                ...
                if (!is_valid) {
                    // This is an invalid or expired session
